...
func _draw() -> void:
	var center := size / 2.0
	var radius: float = min(size.x, size.y) * 0.37
	...
	for index in tiles.size():
		var angle: float = -PI / 2.0 + TAU * float(index) / tiles.size()
	...
	for index in players.size():
		...
		var angle: float = -PI / 2.0 + TAU * float(player.position) / tiles.size()
		var point: Vector2 = center + Vector2(cos(angle), sin(angle)) * radius + Vector2(-12 + (index % 2) * 24, -12 + (index / 2) * 24)
